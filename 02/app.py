...
class EnvCommandBody(BaseModel):
    msg: str = Field(..., description="发送给env004的消息")


@app.post("/api/env/send", summary="向env004发送消息")
async def api_env_send(body: EnvCommandBody):
    result = bemfa_api.send_msg(config.ENV_TOPIC, body.msg.strip())
    if not result.get("ok"):
        return JSONResponse(content=result, status_code=502)
    return {"ok": True, "topic": config.ENV_TOPIC, "msg": body.msg, "raw": result.get("raw")}
