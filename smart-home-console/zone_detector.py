...
class ZoneDetector:
    ...
    def update(self, person_points: Sequence[tuple[float, float]]) -> ZoneUpdate:
        with self._lock:
            people_in_zone = self._count_people(person_points)
            now = self._clock()
    ...
    def _count_people(self, person_points: Sequence[tuple[float, float]]) -> int:
        if self._zone is None:
            return 0
        return sum(1 for point in person_points if self._zone.contains(point))
