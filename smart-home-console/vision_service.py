...
def extract_person_points(
    ...
    """Convert YOLO xyxy person boxes to clamped normalized center points."""
    if boxes is None:
        return []
    ...
    points = []
    for x1, y1, x2, y2, *_rest in rows:
        x = ((float(x1) + float(x2)) / 2.0) / frame_width
        y = ((float(y1) + float(y2)) / 2.0) / frame_height
        ...
        with self._safety_lock:
            ...
            try:
                ...
                if self._zone_detector is not None:
                    ...
                    height, width = frame.shape[:2]
                    person_points = extract_person_points(boxes, width, height)
                    previous_state = self._zone_detector.get_status()["state"]
                    update = self._zone_detector.update(person_points)
