...
class VisionServiceTests(unittest.TestCase):
    ...
    def test_extract_person_points_uses_box_center_and_clamps(self):
        boxes = SafetyBoxes(
            [
                [100.0, 100.0, 300.0, 400.0],
                [-500.0, -20.0, -100.0, -10.0],
                [700.0, 100.0, 900.0, 1200.0],
            ]
        )

        points = extract_person_points(boxes, 640, 480)

        self.assertEqual(points[0], (0.3125, 250.0 / 480.0))
    ...
    def test_camera_disconnect_does_not_call_zone_update_without_a_frame(self):
        class CountingDetector(ZoneDetector):
            ...
            def update(self, person_points):
                self.update_calls += 1
                return super().update(person_points)
    ...
    def test_person_center_inside_zone_triggers_when_feet_are_outside(self):
        service, _detector, repository, alarm, clock, _directory = (
            self.make_safety_service(
                zone=NormalizedZone(0.25, 0.45, 0.125, 0.2)
            )
        )
        ...
        self.assertEqual(len(events), 1)
        event = events[0]
        self.assertEqual(event["max_people"], 1)
        self.assertEqual(alarm.targets, [(True, event["id"])])
